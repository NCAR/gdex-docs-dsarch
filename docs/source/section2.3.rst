...
single dsarch invocation may combine command-line options with one or more
...
As a safeguard against operating on the wrong dataset, input file names must
begin with the dataset number in the format 'dNNNNNN.*', where '*' matches
one or more valid filename characters.
